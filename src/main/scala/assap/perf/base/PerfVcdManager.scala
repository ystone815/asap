...
object PerfVcdManager {
  ...
  def log(scope: String, name: String, value: Long): Unit = {
    ...
    if (oldVal != value) {
      values(id) = value
      // We buffer changes? No, generic VCD expects changes under timestamp.
      // But we need the 'current timestamp' context. 
      // Users usually call log() during step().
      // Ideally, we write changes at the end of the step.
      // For now, we store them in 'values' and flush them when 'step' is called in Sim.
      // Actually, simplest is: User calls update(), we write immediately? 
      // No, VCD requires #timestamp first.
    }
  }
