...
object PerfBaseExample extends App {
  ...
  SimConfig.withWave.compile(new Top).doSim { dut =>
    ...
    // --- 2. Components ---
    // Using 1ns (1000ps) as basic cycle time
    val gen1 = new PerfPacketGenerator("Gen1", output = q1, srcId = 10, rate = 0.3, trace = true)
    val gen2 = new PerfPacketGenerator("Gen2", output = q2, srcId = 20, rate = 0.3, trace = true)
    
    val arbiter = new PerfArbiter[Packet]("Arbiter", inputs = Seq(q1, q2), output = qArb)
    val delayLine = new PerfDelayLine[Packet]("Delay5ns", input = qArb, output = qDelay, latency = 5000) // 5ns
    ...
    println("\n--- Simulation Results ---")
    println(s"SinkA: Received ${sinkA.receivedCount}, Avg Latency: ${if(sinkA.receivedCount>0) sinkA.totalLatency/sinkA.receivedCount else 0} ps")
    println(s"SinkB: Received ${sinkB.receivedCount}, Avg Latency: ${if(sinkB.receivedCount>0) sinkB.totalLatency/sinkB.receivedCount else 0} ps")
